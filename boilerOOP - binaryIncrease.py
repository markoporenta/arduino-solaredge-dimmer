...
#for x, dict in enumerate(inverterM.data):
    ...
    if(inverterM.previousExport >= inverterM.exportmax - inverterM.reserve): #and (controller.temperature < controller.maxtemp): #export is at max/slightly over max, temp is under max
        a = 255 - dimmer.value
        dimmer.value = dimmer.value + math.floor(a/2)
        controller.updateDimmer(controller.ip, dimmer.value)
        dimmer.updateWattage()
    else: #export is under max
        excessValue = math.floor((inverterM.exportmax - inverterM.previousExport) / (dimmer.maxWattage / 255)) # by how many values we should reduce the dimmer
        dimmer.value = max(0, dimmer.value - excessValue)
        controller.updateDimmer(controller.ip, dimmer.value)
        #a = max(0.1, 1 - dimmer.value)
        #dimmer.value = dimmer.value - a/2
        #dimmer.value = max(0, dimmer.value)
        #controller.updateDimmer(controller.ip, dimmer.value)
        #dimmer.updateWattage()
...
"""
    ...
    if (inverterM.previousExport == -1): # if first iteration
        ...
        dimmer.value = max(0, dimmerWattage / dimmer.maxWattage)
"""
